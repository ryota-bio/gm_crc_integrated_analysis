...
#
# This script performs GO Biological Process ORA for
...
#   results/figures/*GO_BP*_dotplot.pdf
#   results/figures/*GO_BP*_dotplot.png
# ============================================================
...
# Use the same x-axis range for all GO-BP dot plots
x_axis_max <- 0.15
...
make_go_dotplot <- function(
  ...
  if (is.null(go_df) || nrow(go_df) == 0) {
    message("[INFO] No GO terms to plot: ", output_prefix)
    return(invisible(NULL))
  }

  required_cols <- c("ID", "Description", "GeneRatio", "Count", "p.adjust")
  missing_cols <- setdiff(required_cols, colnames(go_df))
  ...
  if (nrow(plot_df) == 0) {
    message("[INFO] No GO terms after filtering: ", output_prefix)
    return(invisible(NULL))
  }

  # Increase plot height according to the number of displayed terms
...
run_go_ora_for_one_set <- function(
  entrez_genes,
  universe_entrez,
  comparison_name,
  direction_name
  ...
  if (length(entrez_genes) < 5) {
    message(
      "[INFO] Skipped GO ORA because fewer than 5 mapped genes: ",
      comparison_name,
      " ",
      direction_name
    )
  ...
  go_csv <- file.path(
    ora_dir,
    paste0(comparison_name, "_GO_BP_", direction_name, "_ORA.csv")
  )

  readr::write_csv(go_df, go_csv)
  message("[INFO] Saved: ", go_csv)

  make_go_dotplot(
    go_df = go_df,
    output_prefix = paste0(comparison_name, "_GO_BP_", direction_name, "_byPadj_dotplot"),
    plot_title = paste0(comparison_name, " GO-BP ORA (", direction_name, ", by p.adjust)"),
    top_n = top_n,
