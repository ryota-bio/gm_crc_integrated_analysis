...
# ============================================================
# GO Biological Process ORA only
# HCT116 cells infected with Gemella morbillorum
#
# This script performs GO BP over-representation analysis
...
run_go_ora <- function(deg_file, contrast_name,
                       padj_cutoff = 0.05,
                       log2fc_cutoff = 1) {
  ...
  for (direction in names(gene_sets)) {
    ...
    out_file <- file.path(
      ora_dir,
      paste0(contrast_name, "_GO_BP_", direction, "_ORA.csv")
    )
    ...
    readr::write_csv(ego_df, out_file)
...
message("[INFO] GO BP ORA analysis completed.")
