...
clean_kegg_label <- function(x) {
  x %>%
    str_replace(" - Homo sapiens \\(human\\)$", "") %>%
    str_to_sentence()
}

make_kegg_dotplot <- function(input_file,
                              output_prefix,
                              plot_title,
                              top_n = 15,
                              x_axis_max = 0.20) {
  ...
  plot_df <- df %>%
    ...
    mutate(
      ...
      Description = clean_kegg_label(Description)
    ) %>%
...
make_kegg_dotplot(file.path(ora_dir, "Inf4h_vs_Ctr_KEGG_up_ORA.csv"),
                  "Inf4h_vs_Ctr_KEGG_up_byCount_dotplot",
                  "Inf4h vs Ctr KEGG up")

make_kegg_dotplot(file.path(ora_dir, "Inf4h_vs_Ctr_KEGG_down_ORA.csv"),
                  "Inf4h_vs_Ctr_KEGG_down_byCount_dotplot",
                  "Inf4h vs Ctr KEGG down")

make_kegg_dotplot(file.path(ora_dir, "Inf24h_vs_Ctr_KEGG_up_ORA.csv"),
                  "Inf24h_vs_Ctr_KEGG_up_byCount_dotplot",
                  "Inf24h vs Ctr KEGG up")

make_kegg_dotplot(file.path(ora_dir, "Inf24h_vs_Ctr_KEGG_down_ORA.csv"),
                  "Inf24h_vs_Ctr_KEGG_down_byCount_dotplot",
                  "Inf24h vs Ctr KEGG down")
